<section class="container py-5 mt-sm-2 my-md-4 my-xl-1" data-bs-theme="dark">
    <div style="height: 550px;" class="card zoom-effect border-0 rounded-0 overflow-hidden">
        ...
        <a class="article-body card-body d-flex flex-column justify-content-center text-center align-items-center position-absolute top-0 start-0 w-100 h-100 text-decoration-none z-3 p-4 p-md-5" href="{{route('article', ['section' => $article->section_uri, 'article' => $article->headline_uri])}}">
            <div class="row w-100">
                <div style="max-width: 576px;" @class([
                    "col-12 p-0 p-md-5",
                    "col-lg-6" => isset($text_position) == false || (isset($text_position) && $text_position == 'end'),
        ...
        <div class="row g-0 h-100 bg-dark rounded-0">
            @if (isset($text_position) == false || (isset($text_position) && $text_position == 'end'))
                <div class="col-12 col-md-4 col-lg-6 zoom-effect-wrapper h-100">
          ...
          <div @class([
            ...
            <div style="max-width: 576px;" class="article-body card-body p-4 p-md-5 text-center">
                <a class="d-flex flex-column nav-link stretched-link" href="{{route('article', ['section' => $article->section_uri, 'article' => $article->headline_uri])}}">
                    <p class="text-uppercase text-danger fs-sm serif">{{$article->localizedSection($sections)}}</p>
                    <h3 class="article-title card-title fw-light text-light crop-text-4 serif">{{$article->headline}}</h2>
                    <div class="d-block">
                        <p class="text-secondary fw-light crop-text-3 serif">{{$article->abstract}}</p>
